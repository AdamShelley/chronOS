...
#[cfg(target_os = "macos")]
const APPPLICATION_DIRS: &[&str] = &["/Applications", "/Users/*/Applications"];

#[cfg(target_os = "windows")]
const APPPLICATION_DIRS: &[&str] = &["C:\\Program Files", "C:\\Program Files (x86)"];

#[cfg(target_os = "linux")]
const APPPLICATION_DIRS: &[&str] = &["/usr/bin", "/usr/local/bin", "/opt"];
...
fn is_valid(process: &Process) -> bool {
    ...
    if let Some(exe_path) = process.exe().unwrap().to_str() {
        let is_in_app_dir = APPPLICATION_DIRS
            .iter()
...
fn list_process() -> Vec<AppInfo> {
    ...
    processes.sort_by(|a, b| a.name.to_ascii_lowercase().cmp(&b.name.to_lowercase()));
    processes
}
